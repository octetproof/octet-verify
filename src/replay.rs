//! Cross-check the authenticated envelope **replay-control** fields against the
//! signed proof.
//!
//! In the server-enforced replay design, the ingestion backend enforces
//! single-use / dedup / freshness over values *surfaced on the envelope* —
//! `upload_nonce`, `nullifier`, `signed_timestamp_ms` — treating them as opaque
//! keys without decoding the proof. That is only sound because the **verifier**
//! confirms each surfaced value equals what the proof actually signed. This
//! module is that confirmation.
//!
//! Three bindings:
...
//! stateless verifier's. Extraction from the on-wire envelope (the
//! `ContinuousProofEnvelope` proto in `--envelope` mode; the JSON `replay_control`
//! object in backend-fetch mode) feeds the same check.
...
/// check is independent of the wire representation; the caller fills it from
/// whichever envelope surface it parsed (proto or JSON).
...
/// values as the JSON surface, so both envelope surfaces feed one check.
